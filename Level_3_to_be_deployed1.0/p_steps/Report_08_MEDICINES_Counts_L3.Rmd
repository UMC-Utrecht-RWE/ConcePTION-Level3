...
tab10_med<-fread(paste0(med_dir,"Masked/", "medicines_my_atc_1_masked.csv"))
if(tab10_med[,.N]>0){
...
datatable(head(tab10_med,12000), options = list(scrollX=T))
  ...
  tab10_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_medicines_my_atc_1_masked.csv"))
  if (tab10_med[,.N]>0){
  setcolorder(tab10_med,c("atc_code_1", "count","total", "meaning", "year", "data_access_provider", "data_source"))
      if(tab10_med[,.N]>12000){  
    print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/MEDICINES/medicines_my_atc_1_masked.csv")
...
tab12_med<-fread(paste0(med_dir,"Masked/", "medicines_my_atc_4_masked.csv"))
if(tab12_med[,.N]>0){
...
datatable(tab12_med, options = list(scrollX=T))
  ...
  tab12_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_medicines_my_atc_4_masked.csv"))
  if(tab12_med[,.N]>0){
if(tab12_med[,.N]>12000){
      print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/MEDICINES/medicines_my_atc_4_masked.csv")
    datatable(head(tab12_med,12000), options = list(scrollX=T))
...
if(!is.null(tab12_graph_med)){
tab12_graph_users_med<-tab12_graph_med[,.(meaning,sex, year,atc_4,no_users)]
fig.tab12_users_med<-vector(mode="list", length=length(unique(tab12_graph_users_med[,meaning])))
for(i in 1:length(unique(tab12_graph_users_med[,meaning]))) {
fig.tab12_users_med[[i]]<-ggplotly(ggplot(tab12_graph_users_med[meaning==unique(tab12_graph_users_med[,meaning])[i]], aes(x = year, y = no_users, group=atc_4)) +
                           geom_line(aes(color=atc_4)) +
                           geom_point(aes(color=atc_4)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("Number of subjects by ATC code, year and sex, meaning:", unique(tab12_graph_users_med[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
#Move the x axis title down so it doesn't overlap with the labels
for (i in 1: length(fig.tab12_users_med)){
fig.tab12_users_med[[i]][['x']][['layout']][['annotations']][[1]][['y']] <--0.15
}
}
...
if(!is.null(tab12_graph_med)){
htmltools::tagList(list(fig.tab12_users_med))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
...
#### Graph: Median number of prescriptions by ATC code, sex, year and meaning

```{r tab12_median_graph, echo=FALSE, warning=FALSE}
if(!is.null(tab12_graph_med)){
tab12_graph_median_med<-tab12_graph_med[,.(meaning,sex, year,atc_4,median_records)]
fig.tab12_median_med<-vector(mode="list", length=length(unique(tab12_graph_median_med[,meaning])))
for(i in 1:length(unique(tab12_graph_median_med[,meaning]))) {
fig.tab12_median_med[[i]]<-ggplotly(ggplot(tab12_graph_median_med[meaning==unique(tab12_graph_median_med[,meaning])[i]], aes(x = year, y = median_records, group=atc_4)) +
                           geom_line(aes(color=atc_4)) +
                           geom_point(aes(color=atc_4)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("median records by ATC code, year and sex, meaning:", unique(tab12_graph_median_med[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
#Move the x axis title down so it doesn't overlap with the labels
for (i in 1: length(fig.tab12_median_med)){
fig.tab12_median_med[[i]][['x']][['layout']][['annotations']][[1]][['y']] <--0.15
}
}
...
if(!is.null(tab12_graph_med)){
htmltools::tagList(list(fig.tab12_median_med))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
...
#### Graph: Mean number of prescriptions by ATC code, sex, year and meaning

```{r tab12_mean_graph, echo=FALSE, warning=FALSE}
if(!is.null(tab12_graph_med)){
tab12_graph_mean_med<-tab12_graph_med[,.(meaning,sex, year,atc_4,mean_records)]
fig.tab12_mean_med<-vector(mode="list", length=length(unique(tab12_graph_mean_med[,meaning])))
for(i in 1:length(unique(tab12_graph_mean_med[,meaning]))) {
fig.tab12_mean_med[[i]]<-ggplotly(ggplot(tab12_graph_mean_med[meaning==unique(tab12_graph_mean_med[,meaning])[i]], aes(x = year, y = mean_records, group=atc_4)) +
                           geom_line(aes(color=atc_4)) +
                           geom_point(aes(color=atc_4)) + 
                            facet_wrap(~ sex, ncol=2, scales = "fixed") +
                               ggtitle(paste0("median records by ATC code, year and sex, meaning:", unique(tab12_graph_mean_med[,meaning])[i])) + 
                               xlab("Year") +
                               ylab("Count")+
                             theme_classic() +
                             guides(shape = guide_legend(override.aes = list(size = 0.3))) +
...
#Move the x axis title down so it doesn't overlap with the labels
for (i in 1: length(fig.tab12_mean_med)){
fig.tab12_mean_med[[i]][['x']][['layout']][['annotations']][[1]][['y']] <--0.15
}
}
...
if(!is.null(tab12_graph_med)){
htmltools::tagList(list(fig.tab12_mean_med))
} else {
  print("This table cannot be produced based on the available data.") 
}
```
...
tab13_med<-fread(paste0(med_dir,"Masked/", "medicines_my_atc_7_masked.csv"))
...
if(tab13_med[,.N]>0){
  ...
  tab13_med<-fread(paste0(med_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_medicines_my_atc_7_masked.csv"))
  if(tab13_med[,.N]>0){
