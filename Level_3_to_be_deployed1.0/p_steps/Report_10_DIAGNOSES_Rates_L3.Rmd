...
All diagnoses records with the same date of event are removed. All duplicated records and records within the time lag(for recurrent events) are discarded. The removed number of rows is displayed below.
...
All records that are removed due to exclusion criteria mentioned in the flowchart are displayed below.

```{r excluded_subjects, echo=F}

if (subpopulations_present=="No"){
  if(sum(grepl(list.files(diag_dir), pattern="excluded_people_diagoses.csv"))>0){
    removed_exclusion<-fread(paste0(diag_dir,"excluded_people_diagoses.csv"))
    
    datatable(removed_exclusion, options = list(scrollX=T))
  }
} else {
 if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/")), pattern="excluded_people_diagoses.csv"))>0){
 removed_exclusion<-fread(paste0(diag_dir,subpopulations_names[a],"/excluded_people_diagoses.csv"))
...
Lag time will be removed for all events of interest presented above, otherwise a first event analysis will be performed.    

#### Rates of recurrent events by year, sex and age band

Person years are calculated as the observed period for each subject of the study population stratified by sex, age and year of observation.     
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_yas_recurrent_masked.csv"))>0){
rates_yas_rec<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_yas_recurrent_masked.csv"))
...
datatable(rates_yas_rec[no_records!=0], options = list(scrollX=T))
  ...
  rates_yas_rec<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_yas_recurrent_masked.csv"))>0){
  rates_yas_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_yas_recurrent_masked.csv"))
  setcolorder(rates_yas_rec, c("event_definition","sex","age_band","year","no_records","person_years","rate_per_100_py"))
  if (rates_yas_rec[,.N]>0){
   if (rates_yas_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_yas_recurrent_masked.csv")
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_ys_recurrent_masked.csv"))>0){
rates_ys_rec<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_ys_recurrent_masked.csv"))
...
datatable(rates_ys_rec[no_records!=0], options = list(scrollX=T))
  ...
  rates_ys_rec<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_ys_recurrent_masked.csv"))>0){
  rates_ys_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_ys_recurrent_masked.csv"))
  setcolorder(rates_ys_rec, c("event_definition","sex","year","no_records","person_years","rate_per_100_py"))
  if (rates_ys_rec[,.N]>0){
   if (rates_ys_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_ys_recurrent_masked.csv")
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_y_recurrent_masked.csv"))>0){
rates_y_rec<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_y_recurrent_masked.csv"))
...
datatable(rates_y_rec[no_records!=0], options = list(scrollX=T))
  ...
  rates_y_rec<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_y_recurrent_masked.csv"))>0){
  rates_y_rec<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_y_recurrent_masked.csv"))
  setcolorder(rates_y_rec, c("event_definition","year","no_records","person_years","rate_per_100_py"))
  if (rates_y_rec[,.N]>0){
   if (rates_y_rec[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_y_recurrent_masked.csv")
...
All subjects that had an event before start follow up are removed(separately for each event). The removed number of rows is displayed below.    
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_yas_first_masked.csv"))>0){
rates_yas_first<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_yas_first_masked.csv"))
...
datatable(rates_yas_first[no_records!=0], options = list(scrollX=T))
  ...
  rates_yas_first<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_yas_first_masked.csv"))>0){
  rates_yas_first<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_yas_first_masked.csv"))
  setcolorder(rates_yas_first, c("event_definition","sex","age_band","year","no_records","person_years","rate_per_100_py"))
  if (rates_yas_first[,.N]>0){
   if (rates_yas_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_yas_first_masked.csv")
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_ys_first_masked.csv"))>0){
rates_ys_first<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_ys_first_masked.csv"))
...
datatable(rates_ys_first[no_records!=0], options = list(scrollX=T))
  ...
  rates_ys_first<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_ys_first_masked.csv"))>0){
  rates_ys_first<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_ys_first_masked.csv"))
  setcolorder(rates_ys_first, c("event_definition","sex","year","no_records","person_years","rate_per_100_py"))
  if (rates_ys_first[,.N]>0){
   if (rates_ys_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_ys_first_masked.csv")
...
if (subpopulations_present=="No"){
  if(sum(grepl(list.files(paste0(diag_dir,"Masked/")), pattern="diagnoses_rates_y_first_masked.csv"))>0){
rates_y_first<-fread(paste0(diag_dir,"Masked/", "diagnoses_rates_y_first_masked.csv"))
...
datatable(rates_y_first[no_records!=0], options = list(scrollX=T))
  ...
  rates_y_first<-NULL
    ...
    if(sum(grepl(list.files(paste0(diag_dir,subpopulations_names[a], "/Masked/")), pattern="diagnoses_rates_y_first_masked.csv"))>0){
  rates_y_first<-fread(paste0(diag_dir,subpopulations_names[a], "/Masked/", subpopulations_names[a],"_diagnoses_rates_y_first_masked.csv"))
  setcolorder(rates_y_first, c("event_definition","year","no_records","person_years","rate_per_100_py"))
  if (rates_y_first[,.N]>0){
   if (rates_y_first[,.N]>12000){   
            print("If the table contanins more than 12000 records it can not be displayed in the report. For the complete table, go to g_output/DIAGNOSES/diagnoses_rates_y_first_masked.csv")
